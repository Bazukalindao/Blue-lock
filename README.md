...
local Window = Rayfield:CreateWindow({
   Name = "Dandy's World Hub(By Bazuka)",
...
Rayfield:Notify({
   ...
   Content = "Hi, this is my hub thx for using it :D. If u want to join Dandy Bin Server(Pretty good discord server): https://discord.gg/bZanwy7wqm/If u wanna join My discord Server: https://discord.gg/bZanwy7wqm",
   Duration = 10.5,
   Image = 14594913232,
...
-- Ejecuta la funciÃ³n cada segundo
while true do
    highlightMonsters()
    wait(1)
end
...
-- FunciÃ³n para agregar Highlight a un modelo
local function addHighlightToModel(model)
    ...
    if not model:FindFirstChildOfClass("Highlight") then
        ...
        highlight.FillTransparency = 0.5 -- Transparencia del relleno (0 es completamente sÃ³lido)
...
-- FunciÃ³n para agregar o actualizar el Highlight en un personaje
...
-- FunciÃ³n para aplicar el Highlight a todos los jugadores cada segundo
...
-- Crear el Slider para ajustar la distancia de teleportaciÃ³n
...
-- Crear un botÃ³n para cerrar el GUI
...
closeButton.MouseButton1Click:Connect(function()
    mainFrame.Visible = false -- Oculta el GUI al presionar el botÃ³n cerrar
end)
...
local teleportSpeed = 0.01 -- Hacemos el tiempo entre teletransportes mÃ¡s rÃ¡pido

-- FunciÃ³n para teletransportar al jugador cuando se mueve
local function teleportPlayer()
    local moveDirection = humanoid.MoveDirection
    if moveDirection.Magnitude > 0 then
        -- Calcular la nueva posiciÃ³n de teletransporte
        local newPosition = humanoidRootPart.Position + moveDirection.Unit * teleportDistance
        
        -- Girar al jugador en la direcciÃ³n de movimiento
...
-- FunciÃ³n que ejecuta la teletransportaciÃ³n constantemente
local function startTPWalk()
    while true do
        teleportPlayer()
        wait(teleportSpeed) -- Aumenta la velocidad de la teletransportaciÃ³n
    end
end
...
-- Crear un marco deslizable (ScrollingFrame) con tamaÃ±o restaurado
local mainFrame = Instance.new("ScrollingFrame")
mainFrame.Size = UDim2.new(0.25, 0, 0.35, 0)  -- TamaÃ±o ajustado al 25% del ancho y 35% del alto de la pantalla
...
-- FunciÃ³n para crear un botÃ³n de "correr" con alternancia de velocidad
local function createRunButton(speed)
    ...
    runButton.Draggable = true -- Hacer que el botÃ³n sea arrastrable

    local isRunning = false -- Variable para alternar entre correr y caminar
    local defaultSpeed = 16 -- Velocidad base del jugador

    -- Bucle para actualizar la velocidad cada 0.001 segundos mientras estÃ¡ en modo "correr"
    spawn(function()
        while true do
            if isRunning then
                local player = game.Players.LocalPlayer
            ...
            wait(0.001) -- Esperar 0.001 segundos entre cada actualizaciÃ³n
        end
    end)
    ...
    runButton.MouseButton1Click:Connect(function()
        ...
        if player and player.Character and player.Character:FindFirstChild("Humanoid") then
            if isRunning then
                -- Si estÃ¡ corriendo, volver a la velocidad normal
                player.Character.Humanoid.WalkSpeed = defaultSpeed
                isRunning = false
                runButton.Text = "Run (" .. speed .. ")" -- Texto cuando estÃ¡ detenido
            else
                -- Si estÃ¡ detenido, correr a la velocidad seleccionada
                isRunning = true
                runButton.Text = "Stop" -- Texto cuando estÃ¡ corriendo
            end
        end
    end)

    -- Agregar un botÃ³n para cerrar el botÃ³n de "Run"
    local closeButton = Instance.new("TextButton")
    closeButton.Size = UDim2.new(0.1, 0, 0.1, 0) -- TamaÃ±o aumentado
    closeButton.Position = UDim2.new(0.9, 0, 0, 0) -- Ajustar posiciÃ³n para que estÃ© en la esquina superior derecha
    closeButton.Text = "X"
    closeButton.BackgroundColor3 = Color3.fromRGB(255, 0, 0)
    ...
    closeButton.MouseButton1Click:Connect(function()
        runButton:Destroy() -- Eliminar el botÃ³n de "Run"
    end)
end

-- Crear botÃ³n de velocidad y manejar el clic para cada velocidad
local function createSpeedButton(stars, speed, position)
    ...
    button.MouseButton1Click:Connect(function()
        ...
        createRunButton(speed) -- Crear el botÃ³n de "correr" con la velocidad seleccionada
    end)
end

-- Crear botones de velocidad con diferentes opciones
createSpeedButton("â­", 20, UDim2.new(0.2, 0, 0.2, 0))
createSpeedButton("â­â­", 22.5, UDim2.new(0.2, 0, 0.4, 0))
createSpeedButton("â­â­â­", 25, UDim2.new(0.2, 0, 0.6, 0))
createSpeedButton("â­â­â­â­", 27.5, UDim2.new(0.2, 0, 0.8, 0))
createSpeedButton("â­â­â­â­â­", 30, UDim2.new(0.2, 0, 1, 0)) -- BotÃ³n de 5 estrellas
local screenGui = Instance.new("ScreenGui")
screenGui.Parent = game.Players.LocalPlayer:WaitForChild("PlayerGui")
...
-- Crear botÃ³n para habilitar el salto
...
-- Crear botÃ³n para deshabilitar el salto
...
jumpButton.MouseButton1Click:Connect(function()
    humanoid.JumpPower = 16 -- Establecer una potencia de salto estÃ¡ndar
end)
...
-- Crear un botÃ³n fuera del GUI para abrirlo nuevamente
...
local character = player.Character or player.CharacterAdded:Wait() -- Asegura que el personaje estÃ© disponible
...
if currentRoom then
    ...
    if model then
        ...
        if freeArea then
            ...
            warn("No se encontrÃ³ la carpeta 'FreeArea' dentro del modelo.")
        end
    else
        warn("No se encontrÃ³ un modelo dentro de 'CurrentRoom'.")
    end
else
    warn("No se encontrÃ³ la carpeta 'CurrentRoom' en el Workspace.")
end
   end,
})
...
-- FunciÃ³n para establecer la iluminaciÃ³n solo para el jugador
local function setPlayerLighting()
    ...
    lighting.ClockTime = 12 -- Establece la hora del dÃ­a al mediodÃ­a
end

-- Llama a la funciÃ³n para establecer la iluminaciÃ³n
setPlayerLighting()

-- Opcional: resetea la iluminaciÃ³n cuando el jugador salga
...
-- Bucle para repetir la ejecuciÃ³n del evento cada 0.3 segundos
while true do
    invokeAbilityEvent() -- Ejecutar la funciÃ³n que invoca el evento
                    ...
                    for _, generator in ipairs(generatorsFolder:GetChildren()) do
                        ...
                        if statsFolder then
                            ...
                            if activePlayer and activePlayer.Value == playerModel then
                                ...
                                print("El ActivePlayer estÃ¡ activo para el jugador:", player.Name)
...
local function reduceLag()
    ...
    -- Ajusta la iluminaciÃ³n del juego para reducir el consumo de recursos
    ...
    for _, uiElement in pairs(player.PlayerGui:GetDescendants()) do
        if uiElement:IsA("ImageLabel") or uiElement:IsA("ImageButton") then
            uiElement.ImageTransparency = 1  -- Reduce la visibilidad de las imÃ¡genes en lugar de ocultarlas completamente
        end
    end
end

-- Ejecuta la funciÃ³n de reducciÃ³n de lag
reduceLag()
   end,
})
...
fpsLabel.Size = UDim2.new(0, 200, 0, 50)  -- TamaÃ±o del texto
fpsLabel.Position = UDim2.new(0, 10, 0, 10)  -- PosiciÃ³n en la esquina superior izquierda
...
if not spawnLocation then
    warn("No se encontrÃ³ la SpawnLocation en Workspace.")
    return
end

-- Teletransportar al jugador a la posiciÃ³n de SpawnLocation
rootPart.CFrame = spawnLocation.CFrame + Vector3.new(0, 3, 0) -- AÃ±ade una pequeÃ±a altura para evitar colisiones
...
titleLabel.Text = "Dandy's world player bangerðŸ¤‘"
titleLabel.BackgroundTransparency = 1
...
-- FunciÃ³n para buscar el primer jugador cuyo nombre comienza con el texto ingresado
...
-- Evento cuando se presiona el botÃ³n de confirmaciÃ³n
...
-- Evento cuando se presiona el botÃ³n de detener seguimiento
...
-- FunciÃ³n para seguir al objetivo
local function followTarget()
    if targetPlayer and targetPlayer.Character then
        local targetRoot = targetPlayer.Character:FindFirstChild("HumanoidRootPart")
        if targetRoot then
            -- Calcular la posiciÃ³n detrÃ¡s del objetivo
            ...
            -- Moverse hacia la posiciÃ³n detrÃ¡s del objetivo
...
-- Actualizar la posiciÃ³n cada frame
...
-- Crea la animaciÃ³n
local animation = Instance.new("Animation")
animation.AnimationId = "rbxassetid://17561277632"  -- ID de la animaciÃ³n

-- Carga la animaciÃ³n
local animationTrack = humanoid:LoadAnimation(animation)

-- FunciÃ³n para reproducir o detener la animaciÃ³n segÃºn la velocidad
local function playAnimation(speed)
    if speed == 0 then
        animationTrack:Stop()  -- Detiene la animaciÃ³n si la velocidad es 0
    else
        if not animationTrack.IsPlaying then
            animationTrack:Play()
        end
        animationTrack:AdjustSpeed(speed)  -- Ajusta la velocidad de la animaciÃ³n
    end
end
...
-- Crea la animaciÃ³n
local animation = Instance.new("Animation")
animation.AnimationId = "rbxassetid://18226927388"  -- ID de la animaciÃ³n
...
local function playAnimation()
    animationTrack:Play()  -- Reproduce la animaciÃ³n
end

-- Llama a la funciÃ³n para reproducir la animaciÃ³n
playAnimation()
   end,
})
...
-- Crea la animaciÃ³n
local animation = Instance.new("Animation")
animation.AnimationId = "rbxassetid://18989418012"  -- ID de la animaciÃ³n
...
-- Crea la animaciÃ³n
local animation = Instance.new("Animation")
animation.AnimationId = "rbxassetid://17059625743"  -- ID de la animaciÃ³n
...
-- Cargar la animaciÃ³n en el Humanoid
local animationTrack = humanoid:LoadAnimation(animation)
...
local Button = Tab15:CreateButton({
   Name = "Plushie DandyðŸ§¸(Use Client Sided Dandy)",
   Callback = function()
   local player = game.Players.LocalPlayer
...
local Button = Tab15:CreateButton({
   Name = "ðŸŸ¦ðŸ–¥ï¸Blue VeeðŸ–¥ï¸ðŸŸ¦",
   Callback = function()
   local player = game.Players.LocalPlayer
local playerName = player.Name
